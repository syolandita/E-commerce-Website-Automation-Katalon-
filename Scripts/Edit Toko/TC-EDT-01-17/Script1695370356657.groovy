...
WebUI.uploadFile(findTestObject('Edit Toko/upload foto'), fileUpload)

WebUI.delay(5)

def fileSize = ((WebUI.getAttribute(findTestObject('Edit Toko/upload foto'), 'size')) as int)

if (((fileUpload.endsWith('.gif') || fileUpload.endsWith('.jpg')) || fileUpload.endsWith('.png')) || fileUpload.endsWith(
    '.jpeg')) {
    if (fileSize <= maxSize) {
        WebUI.verifyTextPresent('Ukuran file melebihi batas!', false)
    } else {
